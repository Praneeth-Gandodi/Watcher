...
Every entry point takes the raw battery percentage rather than a ``Robot``.
Per-tick consumption is a few thousandths of a percent, which is below the two
decimal places the wire contract keeps, so reading the value back off a
projected model would freeze the battery in place.
"""
...
class BatteryManager:
    ...
    def drain(
        ...
        *,
        ...
        if status is RobotStatus.CHARGING:
            return min(100.0, battery_percent + self.gain_per_s * elapsed_s)
        if status in {RobotStatus.FAILED, RobotStatus.OFFLINE}:
        ...
        return max(0.0, battery_percent - consumption)
    ...
    def assess(
        ...
        *,
        ...
        if status in {RobotStatus.FAILED, RobotStatus.OFFLINE, RobotStatus.CHARGING}:
            return None

        range_m = self.estimate_range_m(battery_percent)
        task_distance = 0.0
        if task is not None:
            task_distance = hypot(task.target.x - position.x, task.target.y - position.y)
        threshold = (
            self.critical_threshold_percent
            if battery_percent <= self.critical_threshold_percent
            else self.low_threshold_percent
        )
        needs_energy = battery_percent <= self.low_threshold_percent
        cannot_finish = task_distance > range_m
        if not needs_energy and not cannot_finish:
            return None

        first_report = robot_id not in self.announced
        if first_report:
            self.announced[robot_id] = now_s
        elif now_s - self.announced[robot_id] < LOW_BATTERY_EVENT_MEMORY_S:
            return None
    ...
    def build_return_action(
        ...
        *,
        ...
        return RecoveryAction(
            action_id=f"recovery-charge-{robot_id}-{int(now_s * 1000)}",
            action_type=RecoveryActionType.RETURN_TO_CHARGER,
            target_robot_ids=(robot_id,),
            affected_task_ids=(),
            reason=(
                f"battery {battery_percent:.1f}% is below the reserve; "
